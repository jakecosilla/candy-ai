...
func UpsertPostgresActivity(ctx context.Context, jobs []GreenhouseJob) error {
	...
	// 3. Log Sync Run using standard tracking schema. Support sync_run_errors defensively.
	var syncRunID int
	err = db.QueryRow("INSERT INTO sync_runs (start_time, end_time, status, jobs_processed) VALUES (NOW(), NOW(), 'SUCCESS', $1) RETURNING id", len(jobs)).Scan(&syncRunID)
	if err != nil {
		return fmt.Errorf("Failed to log sync_runs tracking execution: %v", err)
	}
